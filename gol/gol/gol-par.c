...
world_print(int** current_world, int height, int width, int lb, int ub)
{
    int i, j, ub_last_proc;
     if(world_rank == 0){
        ...
        /* Process - 0 receives the whole matrix */
...
main(int argc, char *argv[])
{
	...
	/* All processes hold part of the newstate and share their boundary conditions */
	...
	/* Essential part to keep processes synchronized on each iteration */
        MPI_Barrier(MPI_COMM_WORLD);
        
        /* Each process is ready to receive data from other processes*/
