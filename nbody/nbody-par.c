...
static void exchange_forces(struct world *world,struct process *process){

    if(process->rank==0){
        int lb_recv = 0;
        int ub_recv = 0;
        for(int proc = 1; proc < process->num_of_procs; proc++){
            lb_recv = process->ub*proc;
            ub_recv = lb_recv + process->num_of_iters;
            for(int i = lb_recv; i < ub_recv; i++){
                MPI_Recv(&XF(world,i), 1, MPI_DOUBLE, proc, 0, MPI_COMM_WORLD,MPI_STATUS_IGNORE);
                MPI_Recv(&YF(world,i), 1, MPI_DOUBLE, proc, 0, MPI_COMM_WORLD,MPI_STATUS_IGNORE);
            }
        }
    }
    else{
        for(int i = process->lb; i < process->ub ; i++){
            MPI_Send(&XF(world,i), 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
            MPI_Send(&YF(world,i), 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
        }
    }
}

static void exchange_velocities(struct world *world,struct process *process){
    ...
    if(process->rank==0){
        ...
        for(int proc = 1; proc < process->num_of_procs; proc++){
            ...
            ub_recv = lb_recv + process->num_of_iters;
            for(int i = lb_recv; i < ub_recv; i++){
                MPI_Recv(&XV(world,i), 1, MPI_DOUBLE, proc, 0, MPI_COMM_WORLD,MPI_STATUS_IGNORE);
                MPI_Recv(&YV(world,i), 1, MPI_DOUBLE, proc, 0, MPI_COMM_WORLD,MPI_STATUS_IGNORE);
            }
        }
    }
    else{
        for(int i = process->lb; i < process->ub; i++){
            MPI_Send(&XV(world,i), 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
            MPI_Send(&YV(world,i), 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
        }
    }
}

static void
clear_forces(struct world *world,struct process *process)
{
    int b;
    /* Clear force accumulation variables */
    for (b = process->lb; b < world->bodyCt; ++b) {
        YF(world, b) = XF(world, b) = 0;
    }
}

static void
compute_forces(struct world *world,struct process *process)
{
    int b, c;
    MPI_Request req;
    int lb_send = 0;
    ...
    */
    for (b = process->rank; b < world->bodyCt; b+=process->num_of_procs) {
    ...
    int mypos = (process->rank) + process->num_of_procs;
    int proctosend = (process->rank+1);
    for(int i = (process->rank+1); i < world->bodyCt; i++){
    	if(i == mypos){
    		mypos += process->num_of_procs;
    		proctosend++;
    		continue;
    	}
    	else{
    		MPI_Isend(&XV(world,i), 1, MPI_DOUBLE, proctosend, 0, MPI_COMM_WORLD, &req);
       		MPI_Isend(&YV(world,i), 1, MPI_DOUBLE, proctosend, 0, MPI_COMM_WORLD, &req);
    	}
    	proctosend++;
    	if(proctosend == process->num_of_procs){
    		proctosend = 0;
    	}
    }
    mypos = process->rank;
    int i = 0;
    while(mypos < world->bodyCt){
    	for(i; i < mypos; i++){
    		MPI_Recv(&xfr, 1, MPI_DOUBLE, i, 0, MPI_COMM_WORLD,MPI_STATUS_IGNORE);
    	    MPI_Recv(&yfr, 1, MPI_DOUBLE, i, 0, MPI_COMM_WORLD,MPI_STATUS_IGNORE);
   		    XV(world,mypos) += xfr;
        	YV(world,mypos) += yfr;
    	}
    	mypos += process->num_of_procs;
...
static void
compute_velocities(struct world *world,struct process *process)
{
    int b;
...
static void
compute_positions(struct world *world,struct process *process)
{
    int b;
    MPI_Request req;
    ...
    int i = 0;
    for(i; i < myrank; i++){
    	MPI_Isend(&XN(world,i), 1, MPI_DOUBLE, proctosend, 0, MPI_COMM_WORLD, &req);
       	MPI_Isend(&YN(world,i), 1, MPI_DOUBLE, proctosend, 0, MPI_COMM_WORLD, &req);
    }
    i++;
    myrank += process->num_of_procs;
    for(i; i < myrank; i++){
    	MPI_Recv(&XN(world,i), 1, MPI_DOUBLE, i, 0, MPI_COMM_WORLD,MPI_STATUS_IGNORE);
    	MPI_Recv(&YN(world,i), 1, MPI_DOUBLE, i, 0, MPI_COMM_WORLD,MPI_STATUS_IGNORE);
    }
}
...
nr_flops(int n, int steps) {
  // NOTE: make sure that the code here indeed reflects the actual flops
...
main(int argc, char **argv)
{
	...
	int world_rank;	// ranking of processes (0...world_size-1)
    ...
    if (process == NULL) {
        fprintf(stderr, "Cannot calloc(process)\n");
        exit(1);
    }
    ...
    int iterperproc = world->bodyCt / world_size;
    int modulo = world->bodyCt % world_size;
    process->lb = world_rank*iterperproc;
    process->ub = process->lb + iterperproc;
    if((world_rank) == (world_size)-1)
        process->ub += modulo;
    process->rank = world_rank;
    process->num_of_procs = world_size;
    process->num_of_iters = process->ub - process->lb;
    ...
    for (int step = 0; step < steps; step++) {
        ...
        clear_forces(world,process);
        compute_forces(world,process);
        compute_velocities(world,process);        
        compute_positions(world,process);
    ...
    // exchange velocities
    exchange_velocities(world,process);
    //exhange x-force and y-force
    exchange_forces(world,process);
